...
static bool tf_particulate_matter_callback_handler(void *dev, uint8_t fid, TF_Packetbuffer *payload) {
    ...
    switch(fid) {
        ...
        case TF_PARTICULATE_MATTER_CALLBACK_PM_CONCENTRATION: {
            ...
            TF_HalCommon *common = tf_hal_get_common(particulate_matter->tfp.spitfp.hal);
            common->locked = true;
...
int tf_particulate_matter_destroy(TF_ParticulateMatter *particulate_matter) {
    return tf_tfp_destroy(&particulate_matter->tfp);
}
...
int tf_particulate_matter_get_pm_concentration(TF_ParticulateMatter *particulate_matter, uint16_t *ret_pm10, uint16_t *ret_pm25, uint16_t *ret_pm100) {
    if(tf_hal_get_common(particulate_matter->tfp.spitfp.hal)->locked) {
        return TF_E_LOCKED;
    }
    ...
    uint32_t deadline = tf_hal_current_time_us(particulate_matter->tfp.spitfp.hal) + tf_hal_get_common(particulate_matter->tfp.spitfp.hal)->timeout;
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        ...
        *tmp_connected_uid = tf_packetbuffer_read_char(&particulate_matter->tfp.spitfp.recv_buf);
        ...
        if (tmp_connected_uid[0] == 0 && ret_position != NULL) {
            *ret_position = tf_hal_get_port_name(particulate_matter->tfp.spitfp.hal, particulate_matter->tfp.spitfp.port_id);
        }
...
int tf_particulate_matter_callback_tick(TF_ParticulateMatter *particulate_matter, uint32_t timeout_us) {
    return tf_tfp_callback_tick(&particulate_matter->tfp, tf_hal_current_time_us(particulate_matter->tfp.spitfp.hal) + timeout_us);
}
