...
static bool tf_industrial_counter_callback_handler(void *dev, uint8_t fid, TF_Packetbuffer *payload) {
    ...
    switch(fid) {
        ...
        case TF_INDUSTRIAL_COUNTER_CALLBACK_ALL_COUNTER: {
            ...
            TF_HalCommon *common = tf_hal_get_common(industrial_counter->tfp.spitfp.hal);
            common->locked = true;
...
int tf_industrial_counter_destroy(TF_IndustrialCounter *industrial_counter) {
    return tf_tfp_destroy(&industrial_counter->tfp);
}
...
int tf_industrial_counter_get_counter(TF_IndustrialCounter *industrial_counter, uint8_t channel, int64_t *ret_counter) {
    if(tf_hal_get_common(industrial_counter->tfp.spitfp.hal)->locked) {
        return TF_E_LOCKED;
    }
    ...
    uint32_t deadline = tf_hal_current_time_us(industrial_counter->tfp.spitfp.hal) + tf_hal_get_common(industrial_counter->tfp.spitfp.hal)->timeout;
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        ...
        *tmp_connected_uid = tf_packetbuffer_read_char(&industrial_counter->tfp.spitfp.recv_buf);
        ...
        if (tmp_connected_uid[0] == 0 && ret_position != NULL) {
            *ret_position = tf_hal_get_port_name(industrial_counter->tfp.spitfp.hal, industrial_counter->tfp.spitfp.port_id);
        }
...
int tf_industrial_counter_callback_tick(TF_IndustrialCounter *industrial_counter, uint32_t timeout_us) {
    return tf_tfp_callback_tick(&industrial_counter->tfp, tf_hal_current_time_us(industrial_counter->tfp.spitfp.hal) + timeout_us);
}
