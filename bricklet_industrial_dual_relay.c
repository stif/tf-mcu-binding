...
static bool tf_industrial_dual_relay_callback_handler(void *dev, uint8_t fid, TF_Packetbuffer *payload) {
    ...
    switch(fid) {
        ...
        case TF_INDUSTRIAL_DUAL_RELAY_CALLBACK_MONOFLOP_DONE: {
            ...
            TF_HalCommon *common = tf_hal_get_common(industrial_dual_relay->tfp.spitfp.hal);
            common->locked = true;
...
int tf_industrial_dual_relay_destroy(TF_IndustrialDualRelay *industrial_dual_relay) {
    return tf_tfp_destroy(&industrial_dual_relay->tfp);
}
...
int tf_industrial_dual_relay_set_value(TF_IndustrialDualRelay *industrial_dual_relay, bool channel0, bool channel1) {
    if(tf_hal_get_common(industrial_dual_relay->tfp.spitfp.hal)->locked) {
        return TF_E_LOCKED;
    }
    ...
    uint32_t deadline = tf_hal_current_time_us(industrial_dual_relay->tfp.spitfp.hal) + tf_hal_get_common(industrial_dual_relay->tfp.spitfp.hal)->timeout;
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        ...
        *tmp_connected_uid = tf_packetbuffer_read_char(&industrial_dual_relay->tfp.spitfp.recv_buf);
        ...
        if (tmp_connected_uid[0] == 0 && ret_position != NULL) {
            *ret_position = tf_hal_get_port_name(industrial_dual_relay->tfp.spitfp.hal, industrial_dual_relay->tfp.spitfp.port_id);
        }
...
int tf_industrial_dual_relay_callback_tick(TF_IndustrialDualRelay *industrial_dual_relay, uint32_t timeout_us) {
    return tf_tfp_callback_tick(&industrial_dual_relay->tfp, tf_hal_current_time_us(industrial_dual_relay->tfp.spitfp.hal) + timeout_us);
}
