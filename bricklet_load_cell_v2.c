...
static bool tf_load_cell_v2_callback_handler(void *dev, uint8_t fid, TF_Packetbuffer *payload) {
    ...
    switch(fid) {
        ...
        case TF_LOAD_CELL_V2_CALLBACK_WEIGHT: {
            ...
            TF_HalCommon *common = tf_hal_get_common(load_cell_v2->tfp.spitfp.hal);
            common->locked = true;
...
int tf_load_cell_v2_destroy(TF_LoadCellV2 *load_cell_v2) {
    return tf_tfp_destroy(&load_cell_v2->tfp);
}
...
int tf_load_cell_v2_get_weight(TF_LoadCellV2 *load_cell_v2, int32_t *ret_weight) {
    if(tf_hal_get_common(load_cell_v2->tfp.spitfp.hal)->locked) {
        return TF_E_LOCKED;
    }
    ...
    uint32_t deadline = tf_hal_current_time_us(load_cell_v2->tfp.spitfp.hal) + tf_hal_get_common(load_cell_v2->tfp.spitfp.hal)->timeout;
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        ...
        *tmp_connected_uid = tf_packetbuffer_read_char(&load_cell_v2->tfp.spitfp.recv_buf);
        ...
        if (tmp_connected_uid[0] == 0 && ret_position != NULL) {
            *ret_position = tf_hal_get_port_name(load_cell_v2->tfp.spitfp.hal, load_cell_v2->tfp.spitfp.port_id);
        }
...
int tf_load_cell_v2_callback_tick(TF_LoadCellV2 *load_cell_v2, uint32_t timeout_us) {
    return tf_tfp_callback_tick(&load_cell_v2->tfp, tf_hal_current_time_us(load_cell_v2->tfp.spitfp.hal) + timeout_us);
}
