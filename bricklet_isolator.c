...
static bool tf_isolator_callback_handler(void *dev, uint8_t fid, TF_Packetbuffer *payload) {
    ...
    switch(fid) {
        ...
        case TF_ISOLATOR_CALLBACK_STATISTICS: {
            ...
            TF_HalCommon *common = tf_hal_get_common(isolator->tfp.spitfp.hal);
            common->locked = true;
...
int tf_isolator_destroy(TF_Isolator *isolator) {
    return tf_tfp_destroy(&isolator->tfp);
}
...
int tf_isolator_get_statistics(TF_Isolator *isolator, uint32_t *ret_messages_from_brick, uint32_t *ret_messages_from_bricklet, uint16_t *ret_connected_bricklet_device_identifier, char ret_connected_bricklet_uid[8]) {
    if(tf_hal_get_common(isolator->tfp.spitfp.hal)->locked) {
        return TF_E_LOCKED;
    }
    ...
    uint32_t deadline = tf_hal_current_time_us(isolator->tfp.spitfp.hal) + tf_hal_get_common(isolator->tfp.spitfp.hal)->timeout;
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        ...
        *tmp_connected_uid = tf_packetbuffer_read_char(&isolator->tfp.spitfp.recv_buf);
        ...
        if (tmp_connected_uid[0] == 0 && ret_position != NULL) {
            *ret_position = tf_hal_get_port_name(isolator->tfp.spitfp.hal, isolator->tfp.spitfp.port_id);
        }
...
int tf_isolator_callback_tick(TF_Isolator *isolator, uint32_t timeout_us) {
    return tf_tfp_callback_tick(&isolator->tfp, tf_hal_current_time_us(isolator->tfp.spitfp.hal) + timeout_us);
}
