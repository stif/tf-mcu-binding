...
int tf_rgb_led_v2_destroy(TF_RGBLEDV2 *rgb_led_v2) {
    return tf_tfp_destroy(&rgb_led_v2->tfp);
}
...
int tf_rgb_led_v2_set_rgb_value(TF_RGBLEDV2 *rgb_led_v2, uint8_t r, uint8_t g, uint8_t b) {
    if(tf_hal_get_common(rgb_led_v2->tfp.spitfp.hal)->locked) {
        return TF_E_LOCKED;
    }
    ...
    uint32_t deadline = tf_hal_current_time_us(rgb_led_v2->tfp.spitfp.hal) + tf_hal_get_common(rgb_led_v2->tfp.spitfp.hal)->timeout;
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        ...
        *tmp_connected_uid = tf_packetbuffer_read_char(&rgb_led_v2->tfp.spitfp.recv_buf);
        ...
        if (tmp_connected_uid[0] == 0 && ret_position != NULL) {
            *ret_position = tf_hal_get_port_name(rgb_led_v2->tfp.spitfp.hal, rgb_led_v2->tfp.spitfp.port_id);
        }
...
int tf_rgb_led_v2_callback_tick(TF_RGBLEDV2 *rgb_led_v2, uint32_t timeout_us) {
    return tf_tfp_callback_tick(&rgb_led_v2->tfp, tf_hal_current_time_us(rgb_led_v2->tfp.spitfp.hal) + timeout_us);
}
