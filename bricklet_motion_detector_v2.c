...
static bool tf_motion_detector_v2_callback_handler(void *dev, uint8_t fid, TF_Packetbuffer *payload) {
    ...
    switch(fid) {
        ...
        case TF_MOTION_DETECTOR_V2_CALLBACK_MOTION_DETECTED: {
            ...
            TF_HalCommon *common = tf_hal_get_common(motion_detector_v2->tfp.spitfp.hal);
            common->locked = true;
...
int tf_motion_detector_v2_destroy(TF_MotionDetectorV2 *motion_detector_v2) {
    return tf_tfp_destroy(&motion_detector_v2->tfp);
}
...
int tf_motion_detector_v2_get_motion_detected(TF_MotionDetectorV2 *motion_detector_v2, uint8_t *ret_motion) {
    if(tf_hal_get_common(motion_detector_v2->tfp.spitfp.hal)->locked) {
        return TF_E_LOCKED;
    }
    ...
    uint32_t deadline = tf_hal_current_time_us(motion_detector_v2->tfp.spitfp.hal) + tf_hal_get_common(motion_detector_v2->tfp.spitfp.hal)->timeout;
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        ...
        *tmp_connected_uid = tf_packetbuffer_read_char(&motion_detector_v2->tfp.spitfp.recv_buf);
        ...
        if (tmp_connected_uid[0] == 0 && ret_position != NULL) {
            *ret_position = tf_hal_get_port_name(motion_detector_v2->tfp.spitfp.hal, motion_detector_v2->tfp.spitfp.port_id);
        }
...
int tf_motion_detector_v2_callback_tick(TF_MotionDetectorV2 *motion_detector_v2, uint32_t timeout_us) {
    return tf_tfp_callback_tick(&motion_detector_v2->tfp, tf_hal_current_time_us(motion_detector_v2->tfp.spitfp.hal) + timeout_us);
}
