...
static bool tf_can_v2_callback_handler(void *dev, uint8_t fid, TF_Packetbuffer *payload) {
    ...
    switch(fid) {
        ...
        case TF_CAN_V2_CALLBACK_FRAME_READ_LOW_LEVEL: {
            ...
            TF_HalCommon *common = tf_hal_get_common(can_v2->tfp.spitfp.hal);
            common->locked = true;
...
int tf_can_v2_destroy(TF_CANV2 *can_v2) {
    return tf_tfp_destroy(&can_v2->tfp);
}
...
int tf_can_v2_write_frame_low_level(TF_CANV2 *can_v2, uint8_t frame_type, uint32_t identifier, uint8_t data_length, uint8_t data_data[15], bool *ret_success) {
    if(tf_hal_get_common(can_v2->tfp.spitfp.hal)->locked) {
        return TF_E_LOCKED;
    }
    ...
    uint32_t deadline = tf_hal_current_time_us(can_v2->tfp.spitfp.hal) + tf_hal_get_common(can_v2->tfp.spitfp.hal)->timeout;
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        ...
        *tmp_connected_uid = tf_packetbuffer_read_char(&can_v2->tfp.spitfp.recv_buf);
        ...
        if (tmp_connected_uid[0] == 0 && ret_position != NULL) {
            *ret_position = tf_hal_get_port_name(can_v2->tfp.spitfp.hal, can_v2->tfp.spitfp.port_id);
        }
...
int tf_can_v2_callback_tick(TF_CANV2 *can_v2, uint32_t timeout_us) {
    return tf_tfp_callback_tick(&can_v2->tfp, tf_hal_current_time_us(can_v2->tfp.spitfp.hal) + timeout_us);
}
