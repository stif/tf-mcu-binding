...
static bool tf_thermocouple_v2_callback_handler(void *dev, uint8_t fid, TF_Packetbuffer *payload) {
    ...
    switch(fid) {
        ...
        case TF_THERMOCOUPLE_V2_CALLBACK_TEMPERATURE: {
            ...
            TF_HalCommon *common = tf_hal_get_common(thermocouple_v2->tfp.spitfp.hal);
            common->locked = true;
...
int tf_thermocouple_v2_destroy(TF_ThermocoupleV2 *thermocouple_v2) {
    return tf_tfp_destroy(&thermocouple_v2->tfp);
}
...
int tf_thermocouple_v2_get_temperature(TF_ThermocoupleV2 *thermocouple_v2, int32_t *ret_temperature) {
    if(tf_hal_get_common(thermocouple_v2->tfp.spitfp.hal)->locked) {
        return TF_E_LOCKED;
    }
    ...
    uint32_t deadline = tf_hal_current_time_us(thermocouple_v2->tfp.spitfp.hal) + tf_hal_get_common(thermocouple_v2->tfp.spitfp.hal)->timeout;
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        ...
        *tmp_connected_uid = tf_packetbuffer_read_char(&thermocouple_v2->tfp.spitfp.recv_buf);
        ...
        if (tmp_connected_uid[0] == 0 && ret_position != NULL) {
            *ret_position = tf_hal_get_port_name(thermocouple_v2->tfp.spitfp.hal, thermocouple_v2->tfp.spitfp.port_id);
        }
...
int tf_thermocouple_v2_callback_tick(TF_ThermocoupleV2 *thermocouple_v2, uint32_t timeout_us) {
    return tf_tfp_callback_tick(&thermocouple_v2->tfp, tf_hal_current_time_us(thermocouple_v2->tfp.spitfp.hal) + timeout_us);
}
