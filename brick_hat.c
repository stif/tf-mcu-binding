...
static bool tf_hat_callback_handler(void *dev, uint8_t fid, TF_Packetbuffer *payload) {
    ...
    switch(fid) {
        ...
        case TF_HAT_CALLBACK_VOLTAGES: {
            ...
            TF_HalCommon *common = tf_hal_get_common(hat->tfp.spitfp.hal);
            common->locked = true;
...
int tf_hat_set_sleep_mode(TF_HAT *hat, uint32_t power_off_delay, uint32_t power_off_duration, bool raspberry_pi_off, bool bricklets_off, bool enable_sleep_indicator) {
    if(tf_hal_get_common(hat->tfp.spitfp.hal)->locked) {
        return TF_E_LOCKED;
    }
    ...
    uint32_t deadline = tf_hal_current_time_us(hat->tfp.spitfp.hal) + tf_hal_get_common(hat->tfp.spitfp.hal)->timeout;
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        ...
        *tmp_connected_uid = tf_packetbuffer_read_char(&hat->tfp.spitfp.recv_buf);
        ...
        if (tmp_connected_uid[0] == 0 && ret_position != NULL) {
            *ret_position = tf_hal_get_port_name(hat->tfp.spitfp.hal, hat->tfp.spitfp.port_id);
        }
...
int tf_hat_callback_tick(TF_HAT *hat, uint32_t timeout_us) {
    return tf_tfp_callback_tick(&hat->tfp, tf_hal_current_time_us(hat->tfp.spitfp.hal) + timeout_us);
}
