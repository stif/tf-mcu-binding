...
static bool tf_co2_v2_callback_handler(void *dev, uint8_t fid, TF_Packetbuffer *payload) {
    ...
    switch(fid) {
        ...
        case TF_CO2_V2_CALLBACK_ALL_VALUES: {
            ...
            TF_HalCommon *common = tf_hal_get_common(co2_v2->tfp.spitfp.hal);
            common->locked = true;
...
int tf_co2_v2_destroy(TF_CO2V2 *co2_v2) {
    return tf_tfp_destroy(&co2_v2->tfp);
}
...
int tf_co2_v2_get_all_values(TF_CO2V2 *co2_v2, uint16_t *ret_co2_concentration, int16_t *ret_temperature, uint16_t *ret_humidity) {
    if(tf_hal_get_common(co2_v2->tfp.spitfp.hal)->locked) {
        return TF_E_LOCKED;
    }
    ...
    uint32_t deadline = tf_hal_current_time_us(co2_v2->tfp.spitfp.hal) + tf_hal_get_common(co2_v2->tfp.spitfp.hal)->timeout;
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        ...
        *tmp_connected_uid = tf_packetbuffer_read_char(&co2_v2->tfp.spitfp.recv_buf);
        ...
        if (tmp_connected_uid[0] == 0 && ret_position != NULL) {
            *ret_position = tf_hal_get_port_name(co2_v2->tfp.spitfp.hal, co2_v2->tfp.spitfp.port_id);
        }
...
int tf_co2_v2_callback_tick(TF_CO2V2 *co2_v2, uint32_t timeout_us) {
    return tf_tfp_callback_tick(&co2_v2->tfp, tf_hal_current_time_us(co2_v2->tfp.spitfp.hal) + timeout_us);
}
