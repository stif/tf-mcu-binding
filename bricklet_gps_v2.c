...
static bool tf_gps_v2_callback_handler(void *dev, uint8_t fid, TF_Packetbuffer *payload) {
    ...
    switch(fid) {
        ...
        case TF_GPS_V2_CALLBACK_PULSE_PER_SECOND: {
            ...
            TF_HalCommon *common = tf_hal_get_common(gps_v2->tfp.spitfp.hal);
            common->locked = true;
            fn(gps_v2, user_data);
            common->locked = false;
            break;
...
int tf_gps_v2_destroy(TF_GPSV2 *gps_v2) {
    return tf_tfp_destroy(&gps_v2->tfp);
}
...
int tf_gps_v2_get_coordinates(TF_GPSV2 *gps_v2, uint32_t *ret_latitude, char *ret_ns, uint32_t *ret_longitude, char *ret_ew) {
    if(tf_hal_get_common(gps_v2->tfp.spitfp.hal)->locked) {
        return TF_E_LOCKED;
    }
    ...
    uint32_t deadline = tf_hal_current_time_us(gps_v2->tfp.spitfp.hal) + tf_hal_get_common(gps_v2->tfp.spitfp.hal)->timeout;
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        ...
        *tmp_connected_uid = tf_packetbuffer_read_char(&gps_v2->tfp.spitfp.recv_buf);
        ...
        if (tmp_connected_uid[0] == 0 && ret_position != NULL) {
            *ret_position = tf_hal_get_port_name(gps_v2->tfp.spitfp.hal, gps_v2->tfp.spitfp.port_id);
        }
...
int tf_gps_v2_callback_tick(TF_GPSV2 *gps_v2, uint32_t timeout_us) {
    return tf_tfp_callback_tick(&gps_v2->tfp, tf_hal_current_time_us(gps_v2->tfp.spitfp.hal) + timeout_us);
}
