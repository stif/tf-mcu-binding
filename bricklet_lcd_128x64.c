...
static bool tf_lcd_128x64_callback_handler(void *dev, uint8_t fid, TF_Packetbuffer *payload) {
    ...
    switch(fid) {
        ...
        case TF_LCD_128X64_CALLBACK_TOUCH_POSITION: {
            ...
            TF_HalCommon *common = tf_hal_get_common(lcd_128x64->tfp.spitfp.hal);
            common->locked = true;
...
int tf_lcd_128x64_destroy(TF_LCD128x64 *lcd_128x64) {
    return tf_tfp_destroy(&lcd_128x64->tfp);
}
...
int tf_lcd_128x64_write_pixels_low_level(TF_LCD128x64 *lcd_128x64, uint8_t x_start, uint8_t y_start, uint8_t x_end, uint8_t y_end, uint16_t pixels_length, uint16_t pixels_chunk_offset, bool pixels_chunk_data[448]) {
    if(tf_hal_get_common(lcd_128x64->tfp.spitfp.hal)->locked) {
        return TF_E_LOCKED;
    }
    ...
    uint32_t deadline = tf_hal_current_time_us(lcd_128x64->tfp.spitfp.hal) + tf_hal_get_common(lcd_128x64->tfp.spitfp.hal)->timeout;
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        ...
        *tmp_connected_uid = tf_packetbuffer_read_char(&lcd_128x64->tfp.spitfp.recv_buf);
        ...
        if (tmp_connected_uid[0] == 0 && ret_position != NULL) {
            *ret_position = tf_hal_get_port_name(lcd_128x64->tfp.spitfp.hal, lcd_128x64->tfp.spitfp.port_id);
        }
...
int tf_lcd_128x64_callback_tick(TF_LCD128x64 *lcd_128x64, uint32_t timeout_us) {
    return tf_tfp_callback_tick(&lcd_128x64->tfp, tf_hal_current_time_us(lcd_128x64->tfp.spitfp.hal) + timeout_us);
}
