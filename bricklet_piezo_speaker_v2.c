...
static bool tf_piezo_speaker_v2_callback_handler(void *dev, uint8_t fid, TF_Packetbuffer *payload) {
    ...
    switch(fid) {
        ...
        case TF_PIEZO_SPEAKER_V2_CALLBACK_BEEP_FINISHED: {
            ...
            TF_HalCommon *common = tf_hal_get_common(piezo_speaker_v2->tfp.spitfp.hal);
            common->locked = true;
...
int tf_piezo_speaker_v2_destroy(TF_PiezoSpeakerV2 *piezo_speaker_v2) {
    return tf_tfp_destroy(&piezo_speaker_v2->tfp);
}
...
int tf_piezo_speaker_v2_set_beep(TF_PiezoSpeakerV2 *piezo_speaker_v2, uint16_t frequency, uint8_t volume, uint32_t duration) {
    if(tf_hal_get_common(piezo_speaker_v2->tfp.spitfp.hal)->locked) {
        return TF_E_LOCKED;
    }
    ...
    uint32_t deadline = tf_hal_current_time_us(piezo_speaker_v2->tfp.spitfp.hal) + tf_hal_get_common(piezo_speaker_v2->tfp.spitfp.hal)->timeout;
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        ...
        *tmp_connected_uid = tf_packetbuffer_read_char(&piezo_speaker_v2->tfp.spitfp.recv_buf);
        ...
        if (tmp_connected_uid[0] == 0 && ret_position != NULL) {
            *ret_position = tf_hal_get_port_name(piezo_speaker_v2->tfp.spitfp.hal, piezo_speaker_v2->tfp.spitfp.port_id);
        }
...
int tf_piezo_speaker_v2_callback_tick(TF_PiezoSpeakerV2 *piezo_speaker_v2, uint32_t timeout_us) {
    return tf_tfp_callback_tick(&piezo_speaker_v2->tfp, tf_hal_current_time_us(piezo_speaker_v2->tfp.spitfp.hal) + timeout_us);
}
