...
int tf_one_wire_destroy(TF_OneWire *one_wire) {
    return tf_tfp_destroy(&one_wire->tfp);
}
...
int tf_one_wire_search_bus_low_level(TF_OneWire *one_wire, uint16_t *ret_identifier_length, uint16_t *ret_identifier_chunk_offset, uint64_t ret_identifier_chunk_data[7], uint8_t *ret_status) {
    if(tf_hal_get_common(one_wire->tfp.spitfp.hal)->locked) {
        return TF_E_LOCKED;
    }
    ...
    size_t i;
    uint32_t deadline = tf_hal_current_time_us(one_wire->tfp.spitfp.hal) + tf_hal_get_common(one_wire->tfp.spitfp.hal)->timeout;
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        ...
        *tmp_connected_uid = tf_packetbuffer_read_char(&one_wire->tfp.spitfp.recv_buf);
        ...
        if (tmp_connected_uid[0] == 0 && ret_position != NULL) {
            *ret_position = tf_hal_get_port_name(one_wire->tfp.spitfp.hal, one_wire->tfp.spitfp.port_id);
        }
...
int tf_one_wire_callback_tick(TF_OneWire *one_wire, uint32_t timeout_us) {
    return tf_tfp_callback_tick(&one_wire->tfp, tf_hal_current_time_us(one_wire->tfp.spitfp.hal) + timeout_us);
}
