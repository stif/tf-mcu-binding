...
static bool tf_energy_monitor_callback_handler(void *dev, uint8_t fid, TF_Packetbuffer *payload) {
    ...
    switch(fid) {
        ...
        case TF_ENERGY_MONITOR_CALLBACK_ENERGY_DATA: {
            ...
            TF_HalCommon *common = tf_hal_get_common(energy_monitor->tfp.spitfp.hal);
            common->locked = true;
...
int tf_energy_monitor_destroy(TF_EnergyMonitor *energy_monitor) {
    return tf_tfp_destroy(&energy_monitor->tfp);
}
...
int tf_energy_monitor_get_energy_data(TF_EnergyMonitor *energy_monitor, int32_t *ret_voltage, int32_t *ret_current, int32_t *ret_energy, int32_t *ret_real_power, int32_t *ret_apparent_power, int32_t *ret_reactive_power, uint16_t *ret_power_factor, uint16_t *ret_frequency) {
    if(tf_hal_get_common(energy_monitor->tfp.spitfp.hal)->locked) {
        return TF_E_LOCKED;
    }
    ...
    uint32_t deadline = tf_hal_current_time_us(energy_monitor->tfp.spitfp.hal) + tf_hal_get_common(energy_monitor->tfp.spitfp.hal)->timeout;
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        ...
        *tmp_connected_uid = tf_packetbuffer_read_char(&energy_monitor->tfp.spitfp.recv_buf);
        ...
        if (tmp_connected_uid[0] == 0 && ret_position != NULL) {
            *ret_position = tf_hal_get_port_name(energy_monitor->tfp.spitfp.hal, energy_monitor->tfp.spitfp.port_id);
        }
...
int tf_energy_monitor_callback_tick(TF_EnergyMonitor *energy_monitor, uint32_t timeout_us) {
    return tf_tfp_callback_tick(&energy_monitor->tfp, tf_hal_current_time_us(energy_monitor->tfp.spitfp.hal) + timeout_us);
}
