...
static bool tf_analog_in_v3_callback_handler(void *dev, uint8_t fid, TF_Packetbuffer *payload) {
    ...
    switch(fid) {
        ...
        case TF_ANALOG_IN_V3_CALLBACK_VOLTAGE: {
            ...
            TF_HalCommon *common = tf_hal_get_common(analog_in_v3->tfp.spitfp.hal);
            common->locked = true;
...
int tf_analog_in_v3_destroy(TF_AnalogInV3 *analog_in_v3) {
    return tf_tfp_destroy(&analog_in_v3->tfp);
}
...
int tf_analog_in_v3_get_voltage(TF_AnalogInV3 *analog_in_v3, uint16_t *ret_voltage) {
    if(tf_hal_get_common(analog_in_v3->tfp.spitfp.hal)->locked) {
        return TF_E_LOCKED;
    }
    ...
    uint32_t deadline = tf_hal_current_time_us(analog_in_v3->tfp.spitfp.hal) + tf_hal_get_common(analog_in_v3->tfp.spitfp.hal)->timeout;
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        ...
        *tmp_connected_uid = tf_packetbuffer_read_char(&analog_in_v3->tfp.spitfp.recv_buf);
        ...
        if (tmp_connected_uid[0] == 0 && ret_position != NULL) {
            *ret_position = tf_hal_get_port_name(analog_in_v3->tfp.spitfp.hal, analog_in_v3->tfp.spitfp.port_id);
        }
...
int tf_analog_in_v3_callback_tick(TF_AnalogInV3 *analog_in_v3, uint32_t timeout_us) {
    return tf_tfp_callback_tick(&analog_in_v3->tfp, tf_hal_current_time_us(analog_in_v3->tfp.spitfp.hal) + timeout_us);
}
