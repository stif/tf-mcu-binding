...
static bool tf_uv_light_v2_callback_handler(void *dev, uint8_t fid, TF_Packetbuffer *payload) {
    ...
    switch(fid) {
        ...
        case TF_UV_LIGHT_V2_CALLBACK_UVA: {
            ...
            TF_HalCommon *common = tf_hal_get_common(uv_light_v2->tfp.spitfp.hal);
            common->locked = true;
...
int tf_uv_light_v2_destroy(TF_UVLightV2 *uv_light_v2) {
    return tf_tfp_destroy(&uv_light_v2->tfp);
}
...
int tf_uv_light_v2_get_uva(TF_UVLightV2 *uv_light_v2, int32_t *ret_uva) {
    if(tf_hal_get_common(uv_light_v2->tfp.spitfp.hal)->locked) {
        return TF_E_LOCKED;
    }
    ...
    uint32_t deadline = tf_hal_current_time_us(uv_light_v2->tfp.spitfp.hal) + tf_hal_get_common(uv_light_v2->tfp.spitfp.hal)->timeout;
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        ...
        *tmp_connected_uid = tf_packetbuffer_read_char(&uv_light_v2->tfp.spitfp.recv_buf);
        ...
        if (tmp_connected_uid[0] == 0 && ret_position != NULL) {
            *ret_position = tf_hal_get_port_name(uv_light_v2->tfp.spitfp.hal, uv_light_v2->tfp.spitfp.port_id);
        }
...
int tf_uv_light_v2_callback_tick(TF_UVLightV2 *uv_light_v2, uint32_t timeout_us) {
    return tf_tfp_callback_tick(&uv_light_v2->tfp, tf_hal_current_time_us(uv_light_v2->tfp.spitfp.hal) + timeout_us);
}
