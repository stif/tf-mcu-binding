...
static bool tf_compass_callback_handler(void *dev, uint8_t fid, TF_Packetbuffer *payload) {
    ...
    switch(fid) {
        ...
        case TF_COMPASS_CALLBACK_HEADING: {
            ...
            TF_HalCommon *common = tf_hal_get_common(compass->tfp.spitfp.hal);
            common->locked = true;
...
int tf_compass_destroy(TF_Compass *compass) {
    return tf_tfp_destroy(&compass->tfp);
}
...
int tf_compass_get_heading(TF_Compass *compass, int16_t *ret_heading) {
    if(tf_hal_get_common(compass->tfp.spitfp.hal)->locked) {
        return TF_E_LOCKED;
    }
    ...
    uint32_t deadline = tf_hal_current_time_us(compass->tfp.spitfp.hal) + tf_hal_get_common(compass->tfp.spitfp.hal)->timeout;
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        ...
        *tmp_connected_uid = tf_packetbuffer_read_char(&compass->tfp.spitfp.recv_buf);
        ...
        if (tmp_connected_uid[0] == 0 && ret_position != NULL) {
            *ret_position = tf_hal_get_port_name(compass->tfp.spitfp.hal, compass->tfp.spitfp.port_id);
        }
...
int tf_compass_callback_tick(TF_Compass *compass, uint32_t timeout_us) {
    return tf_tfp_callback_tick(&compass->tfp, tf_hal_current_time_us(compass->tfp.spitfp.hal) + timeout_us);
}
