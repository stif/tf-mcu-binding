...
static bool tf_sound_pressure_level_callback_handler(void *dev, uint8_t fid, TF_Packetbuffer *payload) {
    ...
    switch(fid) {
        ...
        case TF_SOUND_PRESSURE_LEVEL_CALLBACK_DECIBEL: {
            ...
            TF_HalCommon *common = tf_hal_get_common(sound_pressure_level->tfp.spitfp.hal);
            common->locked = true;
...
int tf_sound_pressure_level_destroy(TF_SoundPressureLevel *sound_pressure_level) {
    return tf_tfp_destroy(&sound_pressure_level->tfp);
}
...
int tf_sound_pressure_level_get_decibel(TF_SoundPressureLevel *sound_pressure_level, uint16_t *ret_decibel) {
    if(tf_hal_get_common(sound_pressure_level->tfp.spitfp.hal)->locked) {
        return TF_E_LOCKED;
    }
    ...
    uint32_t deadline = tf_hal_current_time_us(sound_pressure_level->tfp.spitfp.hal) + tf_hal_get_common(sound_pressure_level->tfp.spitfp.hal)->timeout;
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        ...
        *tmp_connected_uid = tf_packetbuffer_read_char(&sound_pressure_level->tfp.spitfp.recv_buf);
        ...
        if (tmp_connected_uid[0] == 0 && ret_position != NULL) {
            *ret_position = tf_hal_get_port_name(sound_pressure_level->tfp.spitfp.hal, sound_pressure_level->tfp.spitfp.port_id);
        }
...
int tf_sound_pressure_level_callback_tick(TF_SoundPressureLevel *sound_pressure_level, uint32_t timeout_us) {
    return tf_tfp_callback_tick(&sound_pressure_level->tfp, tf_hal_current_time_us(sound_pressure_level->tfp.spitfp.hal) + timeout_us);
}
