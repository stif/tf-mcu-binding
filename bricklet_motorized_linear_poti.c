...
static bool tf_motorized_linear_poti_callback_handler(void *dev, uint8_t fid, TF_Packetbuffer *payload) {
    ...
    switch(fid) {
        ...
        case TF_MOTORIZED_LINEAR_POTI_CALLBACK_POSITION: {
            ...
            TF_HalCommon *common = tf_hal_get_common(motorized_linear_poti->tfp.spitfp.hal);
            common->locked = true;
...
int tf_motorized_linear_poti_destroy(TF_MotorizedLinearPoti *motorized_linear_poti) {
    return tf_tfp_destroy(&motorized_linear_poti->tfp);
}
...
int tf_motorized_linear_poti_get_position(TF_MotorizedLinearPoti *motorized_linear_poti, uint16_t *ret_position) {
    if(tf_hal_get_common(motorized_linear_poti->tfp.spitfp.hal)->locked) {
        return TF_E_LOCKED;
    }
    ...
    uint32_t deadline = tf_hal_current_time_us(motorized_linear_poti->tfp.spitfp.hal) + tf_hal_get_common(motorized_linear_poti->tfp.spitfp.hal)->timeout;
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        ...
        *tmp_connected_uid = tf_packetbuffer_read_char(&motorized_linear_poti->tfp.spitfp.recv_buf);
        ...
        if (tmp_connected_uid[0] == 0 && ret_position != NULL) {
            *ret_position = tf_hal_get_port_name(motorized_linear_poti->tfp.spitfp.hal, motorized_linear_poti->tfp.spitfp.port_id);
        }
...
int tf_motorized_linear_poti_callback_tick(TF_MotorizedLinearPoti *motorized_linear_poti, uint32_t timeout_us) {
    return tf_tfp_callback_tick(&motorized_linear_poti->tfp, tf_hal_current_time_us(motorized_linear_poti->tfp.spitfp.hal) + timeout_us);
}
