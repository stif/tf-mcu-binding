...
static bool tf_rgb_led_matrix_callback_handler(void *dev, uint8_t fid, TF_Packetbuffer *payload) {
    ...
    switch(fid) {
        ...
        case TF_RGB_LED_MATRIX_CALLBACK_FRAME_STARTED: {
            ...
            TF_HalCommon *common = tf_hal_get_common(rgb_led_matrix->tfp.spitfp.hal);
            common->locked = true;
...
int tf_rgb_led_matrix_destroy(TF_RGBLEDMatrix *rgb_led_matrix) {
    return tf_tfp_destroy(&rgb_led_matrix->tfp);
}
...
int tf_rgb_led_matrix_set_red(TF_RGBLEDMatrix *rgb_led_matrix, uint8_t red[64]) {
    if(tf_hal_get_common(rgb_led_matrix->tfp.spitfp.hal)->locked) {
        return TF_E_LOCKED;
    }
    ...
    uint32_t deadline = tf_hal_current_time_us(rgb_led_matrix->tfp.spitfp.hal) + tf_hal_get_common(rgb_led_matrix->tfp.spitfp.hal)->timeout;
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        ...
        *tmp_connected_uid = tf_packetbuffer_read_char(&rgb_led_matrix->tfp.spitfp.recv_buf);
        ...
        if (tmp_connected_uid[0] == 0 && ret_position != NULL) {
            *ret_position = tf_hal_get_port_name(rgb_led_matrix->tfp.spitfp.hal, rgb_led_matrix->tfp.spitfp.port_id);
        }
...
int tf_rgb_led_matrix_callback_tick(TF_RGBLEDMatrix *rgb_led_matrix, uint32_t timeout_us) {
    return tf_tfp_callback_tick(&rgb_led_matrix->tfp, tf_hal_current_time_us(rgb_led_matrix->tfp.spitfp.hal) + timeout_us);
}
