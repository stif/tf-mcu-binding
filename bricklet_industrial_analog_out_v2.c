...
int tf_industrial_analog_out_v2_destroy(TF_IndustrialAnalogOutV2 *industrial_analog_out_v2) {
    return tf_tfp_destroy(&industrial_analog_out_v2->tfp);
}
...
int tf_industrial_analog_out_v2_set_enabled(TF_IndustrialAnalogOutV2 *industrial_analog_out_v2, bool enabled) {
    if(tf_hal_get_common(industrial_analog_out_v2->tfp.spitfp.hal)->locked) {
        return TF_E_LOCKED;
    }
    ...
    uint32_t deadline = tf_hal_current_time_us(industrial_analog_out_v2->tfp.spitfp.hal) + tf_hal_get_common(industrial_analog_out_v2->tfp.spitfp.hal)->timeout;
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        ...
        *tmp_connected_uid = tf_packetbuffer_read_char(&industrial_analog_out_v2->tfp.spitfp.recv_buf);
        ...
        if (tmp_connected_uid[0] == 0 && ret_position != NULL) {
            *ret_position = tf_hal_get_port_name(industrial_analog_out_v2->tfp.spitfp.hal, industrial_analog_out_v2->tfp.spitfp.port_id);
        }
...
int tf_industrial_analog_out_v2_callback_tick(TF_IndustrialAnalogOutV2 *industrial_analog_out_v2, uint32_t timeout_us) {
    return tf_tfp_callback_tick(&industrial_analog_out_v2->tfp, tf_hal_current_time_us(industrial_analog_out_v2->tfp.spitfp.hal) + timeout_us);
}
