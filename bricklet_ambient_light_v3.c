...
static bool tf_ambient_light_v3_callback_handler(void *dev, uint8_t fid, TF_Packetbuffer *payload) {
    ...
    switch(fid) {
        ...
        case TF_AMBIENT_LIGHT_V3_CALLBACK_ILLUMINANCE: {
            ...
            TF_HalCommon *common = tf_hal_get_common(ambient_light_v3->tfp.spitfp.hal);
            common->locked = true;
...
int tf_ambient_light_v3_destroy(TF_AmbientLightV3 *ambient_light_v3) {
    return tf_tfp_destroy(&ambient_light_v3->tfp);
}
...
int tf_ambient_light_v3_get_illuminance(TF_AmbientLightV3 *ambient_light_v3, uint32_t *ret_illuminance) {
    if(tf_hal_get_common(ambient_light_v3->tfp.spitfp.hal)->locked) {
        return TF_E_LOCKED;
    }
    ...
    uint32_t deadline = tf_hal_current_time_us(ambient_light_v3->tfp.spitfp.hal) + tf_hal_get_common(ambient_light_v3->tfp.spitfp.hal)->timeout;
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        ...
        *tmp_connected_uid = tf_packetbuffer_read_char(&ambient_light_v3->tfp.spitfp.recv_buf);
        ...
        if (tmp_connected_uid[0] == 0 && ret_position != NULL) {
            *ret_position = tf_hal_get_port_name(ambient_light_v3->tfp.spitfp.hal, ambient_light_v3->tfp.spitfp.port_id);
        }
...
int tf_ambient_light_v3_callback_tick(TF_AmbientLightV3 *ambient_light_v3, uint32_t timeout_us) {
    return tf_tfp_callback_tick(&ambient_light_v3->tfp, tf_hal_current_time_us(ambient_light_v3->tfp.spitfp.hal) + timeout_us);
}
