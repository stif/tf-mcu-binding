...
int tf_oled_128x64_v2_destroy(TF_OLED128x64V2 *oled_128x64_v2) {
    return tf_tfp_destroy(&oled_128x64_v2->tfp);
}
...
int tf_oled_128x64_v2_write_pixels_low_level(TF_OLED128x64V2 *oled_128x64_v2, uint8_t x_start, uint8_t y_start, uint8_t x_end, uint8_t y_end, uint16_t pixels_length, uint16_t pixels_chunk_offset, bool pixels_chunk_data[448]) {
    if(tf_hal_get_common(oled_128x64_v2->tfp.spitfp.hal)->locked) {
        return TF_E_LOCKED;
    }
    ...
    uint32_t deadline = tf_hal_current_time_us(oled_128x64_v2->tfp.spitfp.hal) + tf_hal_get_common(oled_128x64_v2->tfp.spitfp.hal)->timeout;
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        ...
        *tmp_connected_uid = tf_packetbuffer_read_char(&oled_128x64_v2->tfp.spitfp.recv_buf);
        ...
        if (tmp_connected_uid[0] == 0 && ret_position != NULL) {
            *ret_position = tf_hal_get_port_name(oled_128x64_v2->tfp.spitfp.hal, oled_128x64_v2->tfp.spitfp.port_id);
        }
...
int tf_oled_128x64_v2_callback_tick(TF_OLED128x64V2 *oled_128x64_v2, uint32_t timeout_us) {
    return tf_tfp_callback_tick(&oled_128x64_v2->tfp, tf_hal_current_time_us(oled_128x64_v2->tfp.spitfp.hal) + timeout_us);
}
