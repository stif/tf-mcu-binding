...
static bool tf_unknown_callback_handler(void *dev, uint8_t fid, TF_Packetbuffer *payload) {
    ...
    switch(fid) {
        ...
        case TF_UNKNOWN_CALLBACK_ENUMERATE: {
            ...
            TF_HalCommon *common = tf_hal_get_common(unknown->tfp.spitfp.hal);
            common->locked = true;
...
int tf_unknown_destroy(TF_Unknown *unknown) {
    return tf_tfp_destroy(&unknown->tfp);
}
...
int tf_unknown_get_spitfp_error_count(TF_Unknown *unknown, uint32_t *ret_error_count_ack_checksum, uint32_t *ret_error_count_message_checksum, uint32_t *ret_error_count_frame, uint32_t *ret_error_count_overflow) {
    if(tf_hal_get_common(unknown->tfp.spitfp.hal)->locked) {
        return TF_E_LOCKED;
    }
    ...
    uint32_t deadline = tf_hal_current_time_us(unknown->tfp.spitfp.hal) + tf_hal_get_common(unknown->tfp.spitfp.hal)->timeout;
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        ...
        *tmp_connected_uid = tf_packetbuffer_read_char(&unknown->tfp.spitfp.recv_buf);
        ...
        if (tmp_connected_uid[0] == 0 && ret_position != NULL) {
            *ret_position = tf_hal_get_port_name(unknown->tfp.spitfp.hal, unknown->tfp.spitfp.port_id);
        }
...
int tf_unknown_callback_tick(TF_Unknown *unknown, uint32_t timeout_us) {
    return tf_tfp_callback_tick(&unknown->tfp, tf_hal_current_time_us(unknown->tfp.spitfp.hal) + timeout_us);
}
