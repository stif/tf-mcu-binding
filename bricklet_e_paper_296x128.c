...
static bool tf_e_paper_296x128_callback_handler(void *dev, uint8_t fid, TF_Packetbuffer *payload) {
    ...
    switch(fid) {
        ...
        case TF_E_PAPER_296X128_CALLBACK_DRAW_STATUS: {
            ...
            TF_HalCommon *common = tf_hal_get_common(e_paper_296x128->tfp.spitfp.hal);
            common->locked = true;
...
int tf_e_paper_296x128_destroy(TF_EPaper296x128 *e_paper_296x128) {
    return tf_tfp_destroy(&e_paper_296x128->tfp);
}
...
int tf_e_paper_296x128_draw(TF_EPaper296x128 *e_paper_296x128) {
    if(tf_hal_get_common(e_paper_296x128->tfp.spitfp.hal)->locked) {
        return TF_E_LOCKED;
    }
    ...
    uint32_t deadline = tf_hal_current_time_us(e_paper_296x128->tfp.spitfp.hal) + tf_hal_get_common(e_paper_296x128->tfp.spitfp.hal)->timeout;
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        ...
        *tmp_connected_uid = tf_packetbuffer_read_char(&e_paper_296x128->tfp.spitfp.recv_buf);
        ...
        if (tmp_connected_uid[0] == 0 && ret_position != NULL) {
            *ret_position = tf_hal_get_port_name(e_paper_296x128->tfp.spitfp.hal, e_paper_296x128->tfp.spitfp.port_id);
        }
...
int tf_e_paper_296x128_callback_tick(TF_EPaper296x128 *e_paper_296x128, uint32_t timeout_us) {
    return tf_tfp_callback_tick(&e_paper_296x128->tfp, tf_hal_current_time_us(e_paper_296x128->tfp.spitfp.hal) + timeout_us);
}
