...
static bool tf_led_strip_v2_callback_handler(void *dev, uint8_t fid, TF_Packetbuffer *payload) {
    ...
    switch(fid) {
        ...
        case TF_LED_STRIP_V2_CALLBACK_FRAME_STARTED: {
            ...
            TF_HalCommon *common = tf_hal_get_common(led_strip_v2->tfp.spitfp.hal);
            common->locked = true;
...
int tf_led_strip_v2_destroy(TF_LEDStripV2 *led_strip_v2) {
    return tf_tfp_destroy(&led_strip_v2->tfp);
}
...
int tf_led_strip_v2_set_led_values_low_level(TF_LEDStripV2 *led_strip_v2, uint16_t index, uint16_t value_length, uint16_t value_chunk_offset, uint8_t value_chunk_data[58]) {
    if(tf_hal_get_common(led_strip_v2->tfp.spitfp.hal)->locked) {
        return TF_E_LOCKED;
    }
    ...
    uint32_t deadline = tf_hal_current_time_us(led_strip_v2->tfp.spitfp.hal) + tf_hal_get_common(led_strip_v2->tfp.spitfp.hal)->timeout;
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        ...
        *tmp_connected_uid = tf_packetbuffer_read_char(&led_strip_v2->tfp.spitfp.recv_buf);
        ...
        if (tmp_connected_uid[0] == 0 && ret_position != NULL) {
            *ret_position = tf_hal_get_port_name(led_strip_v2->tfp.spitfp.hal, led_strip_v2->tfp.spitfp.port_id);
        }
...
int tf_led_strip_v2_callback_tick(TF_LEDStripV2 *led_strip_v2, uint32_t timeout_us) {
    return tf_tfp_callback_tick(&led_strip_v2->tfp, tf_hal_current_time_us(led_strip_v2->tfp.spitfp.hal) + timeout_us);
}
