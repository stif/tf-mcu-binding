...
static bool tf_thermal_imaging_callback_handler(void *dev, uint8_t fid, TF_Packetbuffer *payload) {
    ...
    switch(fid) {
        ...
        case TF_THERMAL_IMAGING_CALLBACK_HIGH_CONTRAST_IMAGE_LOW_LEVEL: {
            ...
            TF_HalCommon *common = tf_hal_get_common(thermal_imaging->tfp.spitfp.hal);
            common->locked = true;
...
int tf_thermal_imaging_destroy(TF_ThermalImaging *thermal_imaging) {
    return tf_tfp_destroy(&thermal_imaging->tfp);
}
...
int tf_thermal_imaging_get_high_contrast_image_low_level(TF_ThermalImaging *thermal_imaging, uint16_t *ret_image_chunk_offset, uint8_t ret_image_chunk_data[62]) {
    if(tf_hal_get_common(thermal_imaging->tfp.spitfp.hal)->locked) {
        return TF_E_LOCKED;
    }
    ...
    size_t i;
    uint32_t deadline = tf_hal_current_time_us(thermal_imaging->tfp.spitfp.hal) + tf_hal_get_common(thermal_imaging->tfp.spitfp.hal)->timeout;
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        ...
        *tmp_connected_uid = tf_packetbuffer_read_char(&thermal_imaging->tfp.spitfp.recv_buf);
        ...
        if (tmp_connected_uid[0] == 0 && ret_position != NULL) {
            *ret_position = tf_hal_get_port_name(thermal_imaging->tfp.spitfp.hal, thermal_imaging->tfp.spitfp.port_id);
        }
...
int tf_thermal_imaging_callback_tick(TF_ThermalImaging *thermal_imaging, uint32_t timeout_us) {
    return tf_tfp_callback_tick(&thermal_imaging->tfp, tf_hal_current_time_us(thermal_imaging->tfp.spitfp.hal) + timeout_us);
}
