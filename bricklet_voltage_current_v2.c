...
static bool tf_voltage_current_v2_callback_handler(void *dev, uint8_t fid, TF_Packetbuffer *payload) {
    ...
    switch(fid) {
        ...
        case TF_VOLTAGE_CURRENT_V2_CALLBACK_CURRENT: {
            ...
            TF_HalCommon *common = tf_hal_get_common(voltage_current_v2->tfp.spitfp.hal);
            common->locked = true;
...
int tf_voltage_current_v2_destroy(TF_VoltageCurrentV2 *voltage_current_v2) {
    return tf_tfp_destroy(&voltage_current_v2->tfp);
}
...
int tf_voltage_current_v2_get_current(TF_VoltageCurrentV2 *voltage_current_v2, int32_t *ret_current) {
    if(tf_hal_get_common(voltage_current_v2->tfp.spitfp.hal)->locked) {
        return TF_E_LOCKED;
    }
    ...
    uint32_t deadline = tf_hal_current_time_us(voltage_current_v2->tfp.spitfp.hal) + tf_hal_get_common(voltage_current_v2->tfp.spitfp.hal)->timeout;
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        ...
        *tmp_connected_uid = tf_packetbuffer_read_char(&voltage_current_v2->tfp.spitfp.recv_buf);
        ...
        if (tmp_connected_uid[0] == 0 && ret_position != NULL) {
            *ret_position = tf_hal_get_port_name(voltage_current_v2->tfp.spitfp.hal, voltage_current_v2->tfp.spitfp.port_id);
        }
...
int tf_voltage_current_v2_callback_tick(TF_VoltageCurrentV2 *voltage_current_v2, uint32_t timeout_us) {
    return tf_tfp_callback_tick(&voltage_current_v2->tfp, tf_hal_current_time_us(voltage_current_v2->tfp.spitfp.hal) + timeout_us);
}
