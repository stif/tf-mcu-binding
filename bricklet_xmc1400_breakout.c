...
static bool tf_xmc1400_breakout_callback_handler(void *dev, uint8_t fid, TF_Packetbuffer *payload) {
    ...
    switch(fid) {
        ...
        case TF_XMC1400_BREAKOUT_CALLBACK_ADC_VALUES: {
            ...
            TF_HalCommon *common = tf_hal_get_common(xmc1400_breakout->tfp.spitfp.hal);
            common->locked = true;
...
int tf_xmc1400_breakout_destroy(TF_XMC1400Breakout *xmc1400_breakout) {
    return tf_tfp_destroy(&xmc1400_breakout->tfp);
}
...
int tf_xmc1400_breakout_set_gpio_config(TF_XMC1400Breakout *xmc1400_breakout, uint8_t port, uint8_t pin, uint8_t mode, uint8_t input_hysteresis, bool output_level) {
    if(tf_hal_get_common(xmc1400_breakout->tfp.spitfp.hal)->locked) {
        return TF_E_LOCKED;
    }
    ...
    uint32_t deadline = tf_hal_current_time_us(xmc1400_breakout->tfp.spitfp.hal) + tf_hal_get_common(xmc1400_breakout->tfp.spitfp.hal)->timeout;
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        ...
        *tmp_connected_uid = tf_packetbuffer_read_char(&xmc1400_breakout->tfp.spitfp.recv_buf);
        ...
        if (tmp_connected_uid[0] == 0 && ret_position != NULL) {
            *ret_position = tf_hal_get_port_name(xmc1400_breakout->tfp.spitfp.hal, xmc1400_breakout->tfp.spitfp.port_id);
        }
...
int tf_xmc1400_breakout_callback_tick(TF_XMC1400Breakout *xmc1400_breakout, uint32_t timeout_us) {
    return tf_tfp_callback_tick(&xmc1400_breakout->tfp, tf_hal_current_time_us(xmc1400_breakout->tfp.spitfp.hal) + timeout_us);
}
