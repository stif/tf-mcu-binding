...
static bool tf_outdoor_weather_callback_handler(void *dev, uint8_t fid, TF_Packetbuffer *payload) {
    ...
    switch(fid) {
        ...
        case TF_OUTDOOR_WEATHER_CALLBACK_STATION_DATA: {
            ...
            TF_HalCommon *common = tf_hal_get_common(outdoor_weather->tfp.spitfp.hal);
            common->locked = true;
...
int tf_outdoor_weather_destroy(TF_OutdoorWeather *outdoor_weather) {
    return tf_tfp_destroy(&outdoor_weather->tfp);
}
...
int tf_outdoor_weather_get_station_identifiers_low_level(TF_OutdoorWeather *outdoor_weather, uint16_t *ret_identifiers_length, uint16_t *ret_identifiers_chunk_offset, uint8_t ret_identifiers_chunk_data[60]) {
    if(tf_hal_get_common(outdoor_weather->tfp.spitfp.hal)->locked) {
        return TF_E_LOCKED;
    }
    ...
    size_t i;
    uint32_t deadline = tf_hal_current_time_us(outdoor_weather->tfp.spitfp.hal) + tf_hal_get_common(outdoor_weather->tfp.spitfp.hal)->timeout;
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        ...
        *tmp_connected_uid = tf_packetbuffer_read_char(&outdoor_weather->tfp.spitfp.recv_buf);
        ...
        if (tmp_connected_uid[0] == 0 && ret_position != NULL) {
            *ret_position = tf_hal_get_port_name(outdoor_weather->tfp.spitfp.hal, outdoor_weather->tfp.spitfp.port_id);
        }
...
int tf_outdoor_weather_callback_tick(TF_OutdoorWeather *outdoor_weather, uint32_t timeout_us) {
    return tf_tfp_callback_tick(&outdoor_weather->tfp, tf_hal_current_time_us(outdoor_weather->tfp.spitfp.hal) + timeout_us);
}
