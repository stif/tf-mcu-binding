...
int tf_analog_out_v3_destroy(TF_AnalogOutV3 *analog_out_v3) {
    return tf_tfp_destroy(&analog_out_v3->tfp);
}
...
int tf_analog_out_v3_set_output_voltage(TF_AnalogOutV3 *analog_out_v3, uint16_t voltage) {
    if(tf_hal_get_common(analog_out_v3->tfp.spitfp.hal)->locked) {
        return TF_E_LOCKED;
    }
    ...
    uint32_t deadline = tf_hal_current_time_us(analog_out_v3->tfp.spitfp.hal) + tf_hal_get_common(analog_out_v3->tfp.spitfp.hal)->timeout;
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        ...
        *tmp_connected_uid = tf_packetbuffer_read_char(&analog_out_v3->tfp.spitfp.recv_buf);
        ...
        if (tmp_connected_uid[0] == 0 && ret_position != NULL) {
            *ret_position = tf_hal_get_port_name(analog_out_v3->tfp.spitfp.hal, analog_out_v3->tfp.spitfp.port_id);
        }
...
int tf_analog_out_v3_callback_tick(TF_AnalogOutV3 *analog_out_v3, uint32_t timeout_us) {
    return tf_tfp_callback_tick(&analog_out_v3->tfp, tf_hal_current_time_us(analog_out_v3->tfp.spitfp.hal) + timeout_us);
}
