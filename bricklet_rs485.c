...
static bool tf_rs485_callback_handler(void *dev, uint8_t fid, TF_Packetbuffer *payload) {
    ...
    switch(fid) {
        ...
        case TF_RS485_CALLBACK_READ_LOW_LEVEL: {
            ...
            TF_HalCommon *common = tf_hal_get_common(rs485->tfp.spitfp.hal);
            common->locked = true;
...
int tf_rs485_destroy(TF_RS485 *rs485) {
    return tf_tfp_destroy(&rs485->tfp);
}
...
int tf_rs485_write_low_level(TF_RS485 *rs485, uint16_t message_length, uint16_t message_chunk_offset, char message_chunk_data[60], uint8_t *ret_message_chunk_written) {
    if(tf_hal_get_common(rs485->tfp.spitfp.hal)->locked) {
        return TF_E_LOCKED;
    }
    ...
    uint32_t deadline = tf_hal_current_time_us(rs485->tfp.spitfp.hal) + tf_hal_get_common(rs485->tfp.spitfp.hal)->timeout;
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        ...
        *tmp_connected_uid = tf_packetbuffer_read_char(&rs485->tfp.spitfp.recv_buf);
        ...
        if (tmp_connected_uid[0] == 0 && ret_position != NULL) {
            *ret_position = tf_hal_get_port_name(rs485->tfp.spitfp.hal, rs485->tfp.spitfp.port_id);
        }
...
int tf_rs485_callback_tick(TF_RS485 *rs485, uint32_t timeout_us) {
    return tf_tfp_callback_tick(&rs485->tfp, tf_hal_current_time_us(rs485->tfp.spitfp.hal) + timeout_us);
}
