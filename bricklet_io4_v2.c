...
static bool tf_io4_v2_callback_handler(void *dev, uint8_t fid, TF_Packetbuffer *payload) {
    ...
    switch(fid) {
        ...
        case TF_IO4_V2_CALLBACK_INPUT_VALUE: {
            ...
            TF_HalCommon *common = tf_hal_get_common(io4_v2->tfp.spitfp.hal);
            common->locked = true;
...
int tf_io4_v2_destroy(TF_IO4V2 *io4_v2) {
    return tf_tfp_destroy(&io4_v2->tfp);
}
...
int tf_io4_v2_set_value(TF_IO4V2 *io4_v2, bool value[4]) {
    if(tf_hal_get_common(io4_v2->tfp.spitfp.hal)->locked) {
        return TF_E_LOCKED;
    }
    ...
    uint32_t deadline = tf_hal_current_time_us(io4_v2->tfp.spitfp.hal) + tf_hal_get_common(io4_v2->tfp.spitfp.hal)->timeout;
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        ...
        *tmp_connected_uid = tf_packetbuffer_read_char(&io4_v2->tfp.spitfp.recv_buf);
        ...
        if (tmp_connected_uid[0] == 0 && ret_position != NULL) {
            *ret_position = tf_hal_get_port_name(io4_v2->tfp.spitfp.hal, io4_v2->tfp.spitfp.port_id);
        }
...
int tf_io4_v2_callback_tick(TF_IO4V2 *io4_v2, uint32_t timeout_us) {
    return tf_tfp_callback_tick(&io4_v2->tfp, tf_hal_current_time_us(io4_v2->tfp.spitfp.hal) + timeout_us);
}
