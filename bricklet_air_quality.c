...
static bool tf_air_quality_callback_handler(void *dev, uint8_t fid, TF_Packetbuffer *payload) {
    ...
    switch(fid) {
        ...
        case TF_AIR_QUALITY_CALLBACK_ALL_VALUES: {
            ...
            TF_HalCommon *common = tf_hal_get_common(air_quality->tfp.spitfp.hal);
            common->locked = true;
...
int tf_air_quality_destroy(TF_AirQuality *air_quality) {
    return tf_tfp_destroy(&air_quality->tfp);
}
...
int tf_air_quality_get_all_values(TF_AirQuality *air_quality, int32_t *ret_iaq_index, uint8_t *ret_iaq_index_accuracy, int32_t *ret_temperature, int32_t *ret_humidity, int32_t *ret_air_pressure) {
    if(tf_hal_get_common(air_quality->tfp.spitfp.hal)->locked) {
        return TF_E_LOCKED;
    }
    ...
    uint32_t deadline = tf_hal_current_time_us(air_quality->tfp.spitfp.hal) + tf_hal_get_common(air_quality->tfp.spitfp.hal)->timeout;
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        ...
        *tmp_connected_uid = tf_packetbuffer_read_char(&air_quality->tfp.spitfp.recv_buf);
        ...
        if (tmp_connected_uid[0] == 0 && ret_position != NULL) {
            *ret_position = tf_hal_get_port_name(air_quality->tfp.spitfp.hal, air_quality->tfp.spitfp.port_id);
        }
...
int tf_air_quality_callback_tick(TF_AirQuality *air_quality, uint32_t timeout_us) {
    return tf_tfp_callback_tick(&air_quality->tfp, tf_hal_current_time_us(air_quality->tfp.spitfp.hal) + timeout_us);
}
