...
static bool tf_dmx_callback_handler(void *dev, uint8_t fid, TF_Packetbuffer *payload) {
    ...
    switch(fid) {
        ...
        case TF_DMX_CALLBACK_FRAME_STARTED: {
            ...
            TF_HalCommon *common = tf_hal_get_common(dmx->tfp.spitfp.hal);
            common->locked = true;
            fn(dmx, user_data);
            common->locked = false;
            break;
...
int tf_dmx_set_dmx_mode(TF_DMX *dmx, uint8_t dmx_mode) {
    if(tf_hal_get_common(dmx->tfp.spitfp.hal)->locked) {
        return TF_E_LOCKED;
    }
    ...
    uint32_t deadline = tf_hal_current_time_us(dmx->tfp.spitfp.hal) + tf_hal_get_common(dmx->tfp.spitfp.hal)->timeout;
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        ...
        *tmp_connected_uid = tf_packetbuffer_read_char(&dmx->tfp.spitfp.recv_buf);
        ...
        if (tmp_connected_uid[0] == 0 && ret_position != NULL) {
            *ret_position = tf_hal_get_port_name(dmx->tfp.spitfp.hal, dmx->tfp.spitfp.port_id);
        }
...
int tf_dmx_callback_tick(TF_DMX *dmx, uint32_t timeout_us) {
    return tf_tfp_callback_tick(&dmx->tfp, tf_hal_current_time_us(dmx->tfp.spitfp.hal) + timeout_us);
}
