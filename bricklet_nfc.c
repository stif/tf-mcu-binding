...
static bool tf_nfc_callback_handler(void *dev, uint8_t fid, TF_Packetbuffer *payload) {
    ...
    switch(fid) {
        ...
        case TF_NFC_CALLBACK_READER_STATE_CHANGED: {
            ...
            TF_HalCommon *common = tf_hal_get_common(nfc->tfp.spitfp.hal);
            common->locked = true;
...
int tf_nfc_set_mode(TF_NFC *nfc, uint8_t mode) {
    if(tf_hal_get_common(nfc->tfp.spitfp.hal)->locked) {
        return TF_E_LOCKED;
    }
    ...
    uint32_t deadline = tf_hal_current_time_us(nfc->tfp.spitfp.hal) + tf_hal_get_common(nfc->tfp.spitfp.hal)->timeout;
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        ...
        *tmp_connected_uid = tf_packetbuffer_read_char(&nfc->tfp.spitfp.recv_buf);
        ...
        if (tmp_connected_uid[0] == 0 && ret_position != NULL) {
            *ret_position = tf_hal_get_port_name(nfc->tfp.spitfp.hal, nfc->tfp.spitfp.port_id);
        }
...
int tf_nfc_callback_tick(TF_NFC *nfc, uint32_t timeout_us) {
    return tf_tfp_callback_tick(&nfc->tfp, tf_hal_current_time_us(nfc->tfp.spitfp.hal) + timeout_us);
}
