...
static bool tf_rs232_v2_callback_handler(void *dev, uint8_t fid, TF_Packetbuffer *payload) {
    ...
    switch(fid) {
        ...
        case TF_RS232_V2_CALLBACK_READ_LOW_LEVEL: {
            ...
            TF_HalCommon *common = tf_hal_get_common(rs232_v2->tfp.spitfp.hal);
            common->locked = true;
...
int tf_rs232_v2_destroy(TF_RS232V2 *rs232_v2) {
    return tf_tfp_destroy(&rs232_v2->tfp);
}
...
int tf_rs232_v2_write_low_level(TF_RS232V2 *rs232_v2, uint16_t message_length, uint16_t message_chunk_offset, char message_chunk_data[60], uint8_t *ret_message_chunk_written) {
    if(tf_hal_get_common(rs232_v2->tfp.spitfp.hal)->locked) {
        return TF_E_LOCKED;
    }
    ...
    uint32_t deadline = tf_hal_current_time_us(rs232_v2->tfp.spitfp.hal) + tf_hal_get_common(rs232_v2->tfp.spitfp.hal)->timeout;
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        ...
        *tmp_connected_uid = tf_packetbuffer_read_char(&rs232_v2->tfp.spitfp.recv_buf);
        ...
        if (tmp_connected_uid[0] == 0 && ret_position != NULL) {
            *ret_position = tf_hal_get_port_name(rs232_v2->tfp.spitfp.hal, rs232_v2->tfp.spitfp.port_id);
        }
...
int tf_rs232_v2_callback_tick(TF_RS232V2 *rs232_v2, uint32_t timeout_us) {
    return tf_tfp_callback_tick(&rs232_v2->tfp, tf_hal_current_time_us(rs232_v2->tfp.spitfp.hal) + timeout_us);
}
