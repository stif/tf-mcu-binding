...
static bool tf_distance_us_v2_callback_handler(void *dev, uint8_t fid, TF_Packetbuffer *payload) {
    ...
    switch(fid) {
        ...
        case TF_DISTANCE_US_V2_CALLBACK_DISTANCE: {
            ...
            TF_HalCommon *common = tf_hal_get_common(distance_us_v2->tfp.spitfp.hal);
            common->locked = true;
...
int tf_distance_us_v2_destroy(TF_DistanceUSV2 *distance_us_v2) {
    return tf_tfp_destroy(&distance_us_v2->tfp);
}
...
int tf_distance_us_v2_get_distance(TF_DistanceUSV2 *distance_us_v2, uint16_t *ret_distance) {
    if(tf_hal_get_common(distance_us_v2->tfp.spitfp.hal)->locked) {
        return TF_E_LOCKED;
    }
    ...
    uint32_t deadline = tf_hal_current_time_us(distance_us_v2->tfp.spitfp.hal) + tf_hal_get_common(distance_us_v2->tfp.spitfp.hal)->timeout;
    ...
    if (result & TF_TICK_PACKET_RECEIVED && error_code == 0) {
        ...
        *tmp_connected_uid = tf_packetbuffer_read_char(&distance_us_v2->tfp.spitfp.recv_buf);
        ...
        if (tmp_connected_uid[0] == 0 && ret_position != NULL) {
            *ret_position = tf_hal_get_port_name(distance_us_v2->tfp.spitfp.hal, distance_us_v2->tfp.spitfp.port_id);
        }
...
int tf_distance_us_v2_callback_tick(TF_DistanceUSV2 *distance_us_v2, uint32_t timeout_us) {
    return tf_tfp_callback_tick(&distance_us_v2->tfp, tf_hal_current_time_us(distance_us_v2->tfp.spitfp.hal) + timeout_us);
}
